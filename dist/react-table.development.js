(function (global, factory) {
  ...
  function mergeProps() {
    ...
    return propList.reduce(function (props, next) {
      var style = next.style,
          ...
          rest = _objectWithoutPropertiesLoose(next, _excluded);
  ...
  function useAsyncDebounce(defaultFn, defaultWait) {
    ...
    return React.useCallback( /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2() {
      var _len2,
          args,
          _key2,
          _args2 = arguments;
  ...
  function reduceMappings(data, mappings) {
    return mappings.reduce(function (data, mapping) {
      return mapping(data);
    }, data);
  }
  ...
  var _excluded$1 = ["initialState", "defaultColumn", "getSubRows", "getRowId", "stateReducer", "useControlledState"];
  ...
  function useInstance$2(instance) {
    ...
    var _React$useMemo = React.useMemo(function () {
      ...
      //partially apply useFilterType and filterTypes to avoid having to pass these around
      var prepareFilter = function prepareFilter(filter) {
        return getFilterMethod(filter, userFilterTypes || {}, filterTypes);
      };

      var columnFilterPairs = filters.map(function (filterOption) {
        var columnId = filterOption.id,
            filterValue = filterOption.value; //Find the column this filter is refering to
        ...
        //Create the filter function for this column
        var filter = produceFilterFromColumn(column, filterValue, prepareFilter);
        return {
          column: column,
          filter: filter
        };
      ...
      //main row filters update their column with their current filter progress
      var mainRowFilters = columnFilterPairs.map(function (_ref) {
        var column = _ref.column,
            filter = _ref.filter;
        return function (rows) {
          var filteredRows = filter(rows);
          Object.assign(column, {
      ...
      //sub row filters DO NOT update the column with their "in progress" filtered values
      var subRowFilters = columnFilterPairs.map(function (_ref2) {
        var filter = _ref2.filter;
        return filter;
      }); //Build a function that recusively evaluates rows starting from a row

      //Build a function that recusively evaluates rows starting from a row
      var filterSubRows = function filterSubRows(row) {
        return reduceMappings(row.subRows, subRowFilters);
      };

      var evaluateRow = function evaluateRow(row, depth) {
        if (depth === void 0) {
          depth = 0;
        }

        return reduceMappings(row, [//expose the current depth to the filter
        function (row) {
          return _extends({}, row, {
            depth: depth
          });
        ...
        function (row) {
          ...
          return _extends({}, row, {
            subRows: ((_row$subRows = row.subRows) == null ? void 0 : _row$subRows.map(function (row) {
              return evaluateRow(row, depth + 1);
            })) || []
          });
        }, //filter and collect the subrows
        function (row) {
          //collect the flat rows before filtering
          var _row$subRows$reduce = row.subRows.reduce(function (_ref3, row) {
            var filteredFlatRows = _ref3[0],
                filteredRowsById = _ref3[1];
            return [[].concat(filteredFlatRows, row.filteredFlatRows), _extends({}, filteredRowsById, row.filteredRowsById)];
          }, [[], {}]),
              subRowsFlat = _row$subRows$reduce[0],
              subRowsById = _row$subRows$reduce[1];

          var filteredSubRows = filterSubRows(row);
          return _extends({}, row, {
            subRows: filteredSubRows,
            prefilteredSubRows: row.subRows,
            filteredFlatRows: [].concat(filteredSubRows, subRowsFlat),
            filteredRowsById: _extends({}, Object.fromEntries(filteredSubRows.map(function (row) {
              return [row.id, row];
            })), subRowsById)
          });
        }]);
      ...
      //evaluate each row
      var evaluatedRows = rows.map(function (row) {
        return evaluateRow(row);
      }); //filter the main rows with thier special filters

      //filter the main rows with thier special filters
      var filteredRows = reduceMappings(evaluatedRows, mainRowFilters); //do this first so we get a lookup table

      //do this first so we get a lookup table
      var filteredMainRowsById = Object.fromEntries(filteredRows.map(function (row) {
        return [row.id, row];
      })); //collect filteredFlatRows, filteredRowsById

      //collect filteredFlatRows, filteredRowsById
      var _evaluatedRows$reduce = evaluatedRows.reduce(function (_ref4, row) {
        var filteredFlatRows = _ref4[0],
            filteredRowsById = _ref4[1];
        return [[].concat(filteredFlatRows, [filteredRowsById[row.id]] || [], row.filteredFlatRows), _extends({}, filteredRowsById, row.filteredRowsById)];
      }, [[], filteredMainRowsById] //since order doesn't matter in objects prepopulate with the main rows
      ),
          filteredFlatRows = _evaluatedRows$reduce[0],
          filteredRowsById = _evaluatedRows$reduce[1];
  ...
  var produceFilterFromColumn = function produceFilterFromColumn(column, filterValue, getFilterMethod) {
    if (!column) return function (rows) {
      return rows;
    };
    var filterMethod = getFilterMethod(column.filter);
    ...
    return function (rows) {
      return filterMethod(rows, [column.id], filterValue);
    };
  };

  var _excluded$2 = ["globalFilter"];
    ...
    if (action.type === actions.setGlobalFilter) {
      ...
      if (shouldAutoRemoveFilter(filterMethod.autoRemove, newFilter)) {
        var globalFilter = state.globalFilter,
            stateWithoutGlobalFilter = _objectWithoutPropertiesLoose(state, _excluded$2);
  ...
  exports.reduceMappings = reduceMappings;
